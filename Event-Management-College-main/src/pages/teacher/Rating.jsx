    ...
    <div className="min-h-full w-full  p-6 overflow-y-auto">
      <div className="max-w-7xl mx-auto overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-bold text-white mb-2">Event Ratings</h1>
            <p className="text-gray-400">View and manage user feedback for events</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {getUniqueEvents().map(eventName => {
              ...
              return (
                ...
                >
                  <h3 className="text-xl font-bold text-white mb-3">{eventName}</h3>
                  <div className="flex items-center gap-3 mb-2">
                    <StarRating value={Math.round(avgRating)} />
                    <span className="text-yellow-400 font-bold text-lg">{avgRating}</span>
                  </div>
                  <p className="text-gray-400 text-sm">
                    {eventRatings.length} rating{eventRatings.length !== 1 ? 's' : ''}
                  </p>
                </motion.div>
              );
            })}
        ...
        <div className="bg-gray-800/50 backdrop-blur-lg border border-gray-700 rounded-xl p-6">
          <h2 className="text-2xl font-bold text-white mb-6">
            {filterEvent === "all" ? "All Ratings" : `${filterEvent} Ratings`}
          </h2>
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">
                        Rated by User ID: {rating.userId}
                      </p>
                    </div>
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <span className="text-yellow-400 font-bold text-lg">{rating.rating}/5</span>
                  </div>
                </motion.div>
              ))}
            </div>
